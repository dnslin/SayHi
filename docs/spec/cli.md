...
`quick complete` transports a Start request plus every gated Quick transition to Core. A completion request without `writes` retains its completed no-change audit outside the repository; `quick show` replays that audit through Core after restart, and `quick archive` transports a Core-valid archive transition. The external audit root defaults to the local user runtime directory and MAY be set with `SAYHI_QUICK_AUDIT_DIR`, which MUST remain outside the repository.

A completion request with a non-empty `writes` array of `{ path, content }` objects creates a durable changed Quick Record. Core applies each write only through the accepted Writer scope, records `quick.json` with its before/after Baselines, exact changed paths, `commit: null`, and a replayable Projection/Event audit containing the Quick intent, route reason, scope, acceptance criteria, and Gate evidence. Core revalidates the final Baseline before completion. `quick show` reads that durable record and `quick archive` moves it into the Project Store archive. Changed Quick completion and archive MUST NOT create a Git commit.
...
`plan record` is available only to an active Build in Plan. It stores the current requirements snapshot, reviewable Plan text, and the frozen Implement Context Manifest identity as immutable hash-named Task evidence. `plan approve` requires a `user` Event actor and the exact returned Plan and Context Manifest identities; it is the only CLI path from Build Plan to Implement. `task advance` MUST reject that transition. `plan reject` leaves the Task in Plan, and stale Context Manifest content rejects approval before any Implement Event is appended.
